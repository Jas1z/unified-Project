          ...
          {users.map(u => (
            <div key={u.id} className="mobile-card">
              <div className="flex justify-between gap-2 mb-2">
                ...
                <Badge label={u.isRevoked ? 'Revoked' : 'Active'} color={u.isRevoked ? 'red' : 'green'} />
              </div>
              <div className="flex flex-wrap gap-2 mb-2">{roleBadge(u.role)}</div>
              <div className="mobile-card-row">
                <span className="mobile-card-label">Hospital</span>
                <span className="text-xs text-slate-600 text-right">{hospName(u.hospitalId)}</span>
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
                <button type="button" onClick={() => { setShowEdit(u); setEditAttrs(u.attributes.join(', ')); }} className="touch-target text-blue-600 text-xs font-bold bg-blue-50 px-3 py-2 rounded-lg">
                  <Edit2 size={13} className="inline mr-1"/>Edit
                </button>
                {!u.isRevoked && (
                  <button type="button" onClick={() => { setShowRevoke(u); setRevokeReason(''); }} className="touch-target text-red-600 text-xs font-bold bg-red-50 px-3 py-2 rounded-lg">
                    <UserX size={13} className="inline mr-1"/>Revoke
                  </button>
                )}
              </div>
            </div>
              ...
              {users.map((u, i) => (
                <tr key={u.id} className={`border-t border-gray-50 ${i % 2 === 1 ? 'bg-gray-50/40' : ''}`}>
                  ...
                  <td className="px-5 py-3 space-x-3">
                    <button type="button" onClick={() => { setShowEdit(u); setEditAttrs(u.attributes.join(', ')); }} className="text-blue-600 hover:text-blue-800 text-xs font-medium min-h-[36px]"><Edit2 size={13} className="inline mr-1"/>Edit</button>
                    {!u.isRevoked && <button type="button" onClick={() => { setShowRevoke(u); setRevokeReason(''); }} className="text-red-500 hover:text-red-700 text-xs font-medium min-h-[36px]"><UserX size={13} className="inline mr-1"/>Revoke</button>}
                  </td>
                </tr>
              ))}
            </tbody>
