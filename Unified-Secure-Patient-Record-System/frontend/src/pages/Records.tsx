  ...
  return (
    <div className="page-stack">
      <div className="toolbar-panel">
        <select value={typeFilter} onChange={e => { setTypeFilter(e.target.value); setPage(1); }} className="input-mobile sm:max-w-[180px]">
          <option value="">All Types</option>
          {RECORD_TYPES.map(t => <option key={t} value={t}>{t.replace(/_/g, ' ')}</option>)}
        </select>
        <select value={hospFilter} onChange={e => { setHospFilter(e.target.value); setPage(1); }} className="input-mobile sm:max-w-[200px]">
          <option value="">All Hospitals</option>
          {hospitals.map(h => <option key={h.id} value={h.id}>{h.name}</option>)}
        </select>
        {canWrite && (
          <button type="button" onClick={() => setShowAdd(true)} className="btn-primary w-full sm:w-auto sm:ml-auto">
      ...
      <DataPanel>
        {loading ? <div className="py-12"><PageSpinner /></div> : records.length === 0 ? (
          <DataPanelEmpty icon={<FileText size={36} />} title="No records found" />
        ) : (
          <>
                ...
                <div key={r.id} className="mobile-card">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">{patientMap[r.patientId] ?? r.patientId.slice(0, 8)}</p>
                    </div>
                    {statusBadge(r.status)}
                  </div>
                  <div className="flex flex-wrap gap-2 mb-2">{recordTypeBadge(r.recordType)}</div>
                  <div className="mobile-card-row">
                    <span className="mobile-card-label">Hospital</span>
                    <span className="text-slate-600 text-right text-xs">{hospName(r.hospitalId)}</span>
                  </div>
                  <div className="mobile-card-row">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {r.status === 'pending' && user?.role !== 'doctor' && (
                      <button type="button" onClick={() => handleApprove(r)} className="touch-target text-green-700 text-xs font-bold bg-green-50 px-3 py-2 rounded-lg">Approve</button>
                    )}
                    <button type="button" disabled={viewLoading} onClick={() => viewRecordDetail(r)}
                      className="touch-target flex items-center gap-1 text-primary-600 text-xs font-bold disabled:opacity-50">
                      <Eye size={14} /> View
                    </button>
                  </div>
                </div>
                  ...
                  {records.map((r, i) => (
                    <tr key={r.id} className={`border-t border-gray-50 hover:bg-blue-50/30 ${i % 2 === 1 ? 'bg-gray-50/40' : ''}`}>
                      ...
                      <td className="px-5 py-3 flex gap-2">
                        {r.status === 'pending' && user?.role !== 'doctor' && (
                          <button type="button" onClick={() => handleApprove(r)} className="text-green-600 hover:text-green-800 text-xs font-medium bg-green-50 px-2 py-1 rounded min-h-[36px]">Approve</button>
                        )}
                        <button type="button" disabled={viewLoading} onClick={() => viewRecordDetail(r)}
                          className="flex items-center gap-1 text-blue-600 hover:text-blue-800 text-xs font-medium disabled:opacity-50 min-h-[36px]">
                          <Eye size={13} /> View
                        </button>
                      </td>
                    </tr>
            ...
            <TablePagination page={page} total={total} limit={LIMIT} onPrev={() => setPage(p => p - 1)} onNext={() => setPage(p => p + 1)} />
          </>
        )}
      </DataPanel>
